...
// Pdqsort sorts the given array of ints using golang's implementation
func Pdqsort(ctx context.Context,arr[]int) {
	n:=len(arr)
	pdqsortOrdered(ctx,arr,0,n,bits.Len(uint(n)))
}
...
func pdqsortOrdered[E cmp.Ordered](ctx context.Context,data []E, a, b, limit int) {
	...
	for {
		...
		if leftLen < rightLen {
			...
			pdqsortOrdered(ctx, data, a, mid, limit)
			a = mid + 1
		} else {
			wasBalanced = rightLen >= balanceThreshold
			pdqsortOrdered(ctx,data, mid+1, b, limit)
			b = mid
		}
	}
}
